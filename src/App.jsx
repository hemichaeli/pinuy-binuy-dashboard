...
const SEVERITY_STYLE = {
  ...
  low: { bg: "rgba(107,114,128,0.12)", border: "#6b7280", dot: "#6b7280" },
  info: { bg: "rgba(59,130,246,0.12)", border: "#3b82f6", dot: "#3b82f6" },
};
...
function Badge({ children, color = "#60a5fa", size = "sm" }) {
  ...
  return (
    <span style={{
      ...
      background: `${color}18`, color, fontSize: fs, fontWeight: 600,
      letterSpacing: 0.3, border: `1px solid ${color}30`
    }}>{children}</span>
  );
}
...
function StatCard({ label, value, sub, accent = "#60a5fa", icon }) {
  return (
    ...
    >
      <div style={{ fontSize: 12, color: "#94a3b8", letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 8 }}>
        {icon && <span style={{ marginLeft: 6 }}>{icon}</span>}{label}
      </div>
...
function IAIBar({ score, width = 120, height = 8 }) {
  const cat = IAI_CAT(score);
  const color = IAI_COLORS[cat];
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <div style={{ width, height, background: "rgba(255,255,255,0.06)", borderRadius: height / 2, overflow: "hidden" }}>
        <div style={{ width: `${score}%`, height: "100%", background: color, borderRadius: height / 2, transition: "width 0.6s ease" }} />
      </div>
      <span style={{ color, fontWeight: 700, fontSize: 14, fontFamily: "'JetBrains Mono', monospace" }}>{score}</span>
    </div>
  );
}
...
function StatusPill({ status }) {
  const colorMap = {
    declared: "#8b5cf6", planning: "#6366f1", pre_deposit: "#3b82f6",
    deposited: "#0ea5e9", approved: "#10b981", permit: "#22d3ee", construction: "#f59e0b"
  };
  const c = colorMap[status] || "#6b7280";
  return <Badge color={c}>{STATUS_HE[status] || status}</Badge>;
}
        ...
        return (
          <div key={s} style={{ display: "flex", alignItems: "center" }}>
            <div style={{
              ...
              background: active ? (isCurrent ? "#10b981" : "#10b98180") : "rgba(255,255,255,0.1)",
            ...
            {i < STATUS_FLOW.length - 1 && (
              <div style={{
                width: 24, height: 2,
                background: i < idx ? "#10b98160" : "rgba(255,255,255,0.06)"
              }} />
            )}
          </div>
        );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
        <StatCard icon={"\uD83C\uDFD7"} label={"\u05DE\u05EA\u05D7\u05DE\u05D9\u05DD"} value={health?.complexes || 0} accent="#60a5fa" />
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                {["\u05E6\u05D9\u05D5\u05DF","IAI","\u05E4\u05E8\u05D5\u05D9\u05E7\u05D8","\u05E2\u05D9\u05E8","\u05E1\u05D8\u05D8\u05D5\u05E1","\u05D9\u05D7\u05F4\u05D3","\u05D9\u05D6\u05DD","\u05DE\u05D5\u05D3\u05E2\u05D5\u05EA"].map((h) => (
                  <th key={h} style={{ padding: "10px 14px", textAlign: "right", color: "#64748b", fontWeight: 500, fontSize: 11, letterSpacing: 0.5, textTransform: "uppercase" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {opportunities.map((p, i) => (
                <tr key={p.id} onClick={() => onNavigate("detail", p.id)}
                ...
                >
                  ...
                  <td style={{ padding: "12px 14px" }}>
                    {parseInt(p.active_listings) > 0 ? <Badge color="#34d399">{p.active_listings}</Badge> : <span style={{ color: "#475569" }}>0</span>}
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {alerts?.alerts?.length > 0 && (
        <div style={{ background: "rgba(255,255,255,0.02)", borderRadius: 16, border: "1px solid rgba(255,255,255,0.06)", padding: "18px 22px" }}>
          <h2 style={{ margin: "0 0 14px", fontSize: 16, fontWeight: 600, color: "#e2e8f0" }}>
            {"\uD83D\uDD14"} {"\u05D4\u05EA\u05E8\u05D0\u05D5\u05EA \u05D0\u05D7\u05E8\u05D5\u05E0\u05D5\u05EA"}
          </h2>
          {alerts.alerts.map((a) => {
...
function OpportunitiesPage({ onNavigate }) {
  ...
  const filtered = raw
    .filter((p) => (!filterCity || p.city === filterCity) && (!filterStatus || p.status === filterStatus))
    .sort((a, b) => {
  ...
  const SortHead = ({ k, children }) => (
    <th onClick={() => toggleSort(k)} style={{ padding: "10px 14px", textAlign: "right", color: sortKey === k ? "#60a5fa" : "#64748b", fontWeight: 500, fontSize: 11, cursor: "pointer", letterSpacing: 0.5, textTransform: "uppercase", userSelect: "none", whiteSpace: "nowrap" }}>
    ...
    <div>
      <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} style={selectStyle}>
          ...
          {statuses.map((s) => <option key={s} value={s}>{STATUS_HE[s] || s}</option>)}
        </select>
      </div>
      ...
      <div style={{ background: "rgba(255,255,255,0.02)", borderRadius: 16, border: "1px solid rgba(255,255,255,0.06)", overflow: "hidden" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                <SortHead k="iai_score">IAI</SortHead>
                <SortHead k="name">{"\u05E4\u05E8\u05D5\u05D9\u05E7\u05D8"}</SortHead>
                <SortHead k="city">{"\u05E2\u05D9\u05E8"}</SortHead>
                <SortHead k="status">{"\u05E1\u05D8\u05D8\u05D5\u05E1"}</SortHead>
                <SortHead k="planned_units">{"\u05D9\u05D7\u05F4\u05D3 \u05DE\u05EA\u05D5\u05DB\u05E0\u05E0\u05D5\u05EA"}</SortHead>
                <SortHead k="existing_units">{"\u05D9\u05D7\u05F4\u05D3 \u05E7\u05D9\u05D9\u05DE\u05D5\u05EA"}</SortHead>
                <th style={{ padding: "10px 14px", textAlign: "right", color: "#64748b", fontWeight: 500, fontSize: 11, letterSpacing: 0.5, textTransform: "uppercase" }}>{"\u05D9\u05D6\u05DD"}</th>
                <th style={{ padding: "10px 14px", textAlign: "right", color: "#64748b", fontWeight: 500, fontSize: 11, letterSpacing: 0.5, textTransform: "uppercase" }}>{"\u05DE\u05D5\u05D3\u05E2\u05D5\u05EA"}</th>
                <th style={{ padding: "10px 14px", textAlign: "right", color: "#64748b", fontWeight: 500, fontSize: 11, letterSpacing: 0.5, textTransform: "uppercase" }}>{"\u05D4\u05DE\u05DC\u05E6\u05D4"}</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((p) => (
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
        <div>
          <button onClick={() => onNavigate("opportunities")} style={{ background: "none", border: "none", color: "#60a5fa", cursor: "pointer", fontSize: 13, padding: 0, marginBottom: 8 }}>&rarr; {"\u05D7\u05D6\u05E8\u05D4 \u05DC\u05D4\u05D6\u05D3\u05DE\u05E0\u05D5\u05D9\u05D5\u05EA"}</button>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{ textAlign: "center" }}>
            <div style={{ fontSize: 42, fontWeight: 800, color: IAI_COLORS[IAI_CAT(c.iai_score)], fontFamily: "'JetBrains Mono', monospace", lineHeight: 1 }}>{c.iai_score}</div>
            <div style={{ fontSize: 11, color: "#64748b", marginTop: 4 }}>IAI Score</div>
          </div>
      ...
      {ls.length > 0 && (
        <div style={{ background: "rgba(255,255,255,0.02)", borderRadius: 14, border: "1px solid rgba(255,255,255,0.06)", overflow: "hidden" }}>
          <div style={{ padding: "16px 22px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
            <h3 style={{ margin: 0, fontSize: 14, color: "#e2e8f0", fontWeight: 600 }}>{"\uD83C\uDFE0"} {"\u05DE\u05D5\u05D3\u05E2\u05D5\u05EA \u05E4\u05E2\u05D9\u05DC\u05D5\u05EA"} ({ls.length})</h3>
          </div>
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                  {["\u05DB\u05EA\u05D5\u05D1\u05EA","\u05D7\u05D3\u05E8\u05D9\u05DD","\u05E9\u05D8\u05D7 (m\u00B2)","\u05DE\u05D7\u05D9\u05E8","\u05DC\u05DE\u05F4\u05E8","SSI","\u05DE\u05E7\u05D5\u05E8"].map((h) => (
                    <th key={h} style={{ padding: "10px 14px", textAlign: "right", color: "#64748b", fontWeight: 500, fontSize: 11 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
...
function AllProjectsPage({ onNavigate }) {
  ...
  const filtered = projects.filter((p) =>
    (!search || p.name.includes(search) || (p.developer && p.developer.includes(search))) &&
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 14 }}>
        {filtered.map((p) => (
          <div key={p.id} onClick={() => onNavigate("detail", p.id)} style={{
            background: "rgba(255,255,255,0.02)", borderRadius: 14, border: "1px solid rgba(255,255,255,0.06)",
          ...
          >
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
              ...
              {p.developer && <span style={{ color: "#64748b", fontSize: 11 }}>{p.developer}</span>}
            </div>
            {p.planned_units && (
              <div style={{ marginTop: 8, color: "#64748b", fontSize: 12 }}>
                {fmt(p.existing_units)} &rarr; {fmt(p.planned_units)} {"\u05D9\u05D7\u05F4\u05D3"}
              </div>
            )}
          </div>
        ))}
...
function AlertsPage() {
  ...
  return (
    <div>
      ...
      {alerts.length === 0 ? (
        <div style={{ textAlign: "center", padding: 60, color: "#475569" }}>{"\u05D0\u05D9\u05DF \u05D4\u05EA\u05E8\u05D0\u05D5\u05EA"}</div>
      ) : alerts.map((a) => {
        const s = SEVERITY_STYLE[a.severity] || SEVERITY_STYLE.info;
        return (
          <div key={a.id} style={{
            padding: "14px 18px", marginBottom: 10, borderRadius: 12, background: s.bg,
            borderRight: `3px solid ${s.border}`, display: "flex", alignItems: "center", gap: 12,
            opacity: a.is_read ? 0.5 : 1, transition: "opacity 0.2s"
          }}>
            <div style={{ width: 10, height: 10, borderRadius: "50%", background: s.dot, flexShrink: 0 }} />
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 600, color: "#e2e8f0", fontSize: 13 }}>{a.title}</div>
              <div style={{ color: "#94a3b8", fontSize: 12, marginTop: 2 }}>{a.message}</div>
              <div style={{ color: "#475569", fontSize: 11, marginTop: 4 }}>{new Date(a.created_at).toLocaleDateString("he-IL")} | {a.complex_name} ({a.city})</div>
            </div>
            {!a.is_read && (
              <button onClick={() => markRead(a.id)} style={{ background: "none", border: "1px solid rgba(255,255,255,0.1)", color: "#94a3b8", padding: "4px 10px", borderRadius: 6, cursor: "pointer", fontSize: 11 }}>{"\u2713"}</button>
            )}
          </div>
        );
      })}
...
function ScanPage() {
  ...
  const triggerScan = async () => {
    setScanning(true);
    try {
      await fetch(`${API}/api/scheduler/run`, { method: "POST" });
      setTimeout(() => { reloadSched(); reloadScans(); setScanning(false); }, 5000);
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        ...
        <button onClick={triggerScan} disabled={scanning || sched?.isRunning} style={{
          background: scanning ? "rgba(245,158,11,0.2)" : "rgba(16,185,129,0.15)",
          border: `1px solid ${scanning ? "#f59e0b50" : "#10b98150"}`,
          color: scanning ? "#f59e0b" : "#10b981",
          padding: "8px 18px", borderRadius: 10, cursor: scanning ? "wait" : "pointer",
          fontSize: 13, fontWeight: 600
        }}>{scanning ? "\u05E1\u05D5\u05E8\u05E7..." : "\u05D4\u05E4\u05E2\u05DC \u05E1\u05E8\u05D9\u05E7\u05D4 \u05D9\u05D3\u05E0\u05D9\u05EA"}</button>
      </div>
      ...
      {sched && (
        <div style={{ background: "rgba(255,255,255,0.02)", borderRadius: 14, border: "1px solid rgba(255,255,255,0.06)", padding: "18px 22px", marginBottom: 20 }}>
          ...
          {sched.lastRun && (
            <div style={{ marginTop: 12, padding: "10px 14px", background: "rgba(255,255,255,0.02)", borderRadius: 8, fontSize: 12, color: "#94a3b8" }}>
              <strong>{"\u05E8\u05D9\u05E6\u05D4 \u05D0\u05D7\u05E8\u05D5\u05E0\u05D4"}:</strong> {sched.lastRun.summary || sched.lastRun.error || "N/A"}
            </div>
          )}
        </div>
      )}
      ...
      <div style={{ background: "rgba(255,255,255,0.02)", borderRadius: 14, border: "1px solid rgba(255,255,255,0.06)", overflow: "hidden" }}>
        <div style={{ padding: "14px 22px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          <h3 style={{ margin: 0, fontSize: 14, color: "#e2e8f0" }}>{"\u05D4\u05D9\u05E1\u05D8\u05D5\u05E8\u05D9\u05D9\u05EA \u05E1\u05E8\u05D9\u05E7\u05D5\u05EA"}</h3>
        </div>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
          <thead>
            <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
              {["#","\u05E1\u05D5\u05D2","\u05E1\u05D8\u05D8\u05D5\u05E1","\u05E0\u05E1\u05E8\u05E7\u05D5","\u05E2\u05E1\u05E7\u05D0\u05D5\u05EA","\u05DE\u05D5\u05D3\u05E2\u05D5\u05EA","\u05D4\u05EA\u05E8\u05D0\u05D5\u05EA","\u05EA\u05D0\u05E8\u05D9\u05DA"].map((h) => (
                <th key={h} style={{ padding: "8px 12px", textAlign: "right", color: "#64748b", fontWeight: 500, fontSize: 11 }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
...
const selectStyle = {
  ...
  color: "#e2e8f0", padding: "6px 12px", borderRadius: 8, fontSize: 12, outline: "none"
};
...
export default function App() {
  ...
  return (
    <div dir="rtl" style={{
      ...
      fontFamily: "'Rubik', 'Segoe UI', sans-serif", color: "#e2e8f0",
      direction: "rtl"
    }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Rubik:wght@300;400;500;600;700;800&family=JetBrains+Mono:wght@400;500;600;700;800&display=swap');
        @keyframes spin { to { transform: rotate(360deg); } }
        * { box-sizing: border-box; }
        ::-webkit-scrollbar { width: 6px; height: 6px; }
        ::-webkit-scrollbar-track { background: transparent; }
        ::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.1); border-radius: 3px; }
        body { margin: 0; padding: 0; background: #0c0f1a; }
        table { direction: rtl; }
      `}</style>

      <div style={{
        position: "fixed", top: 0, left: 0, right: 0, bottom: 0, pointerEvents: "none", zIndex: 0,
        background: "radial-gradient(ellipse at 20% 0%, rgba(59,130,246,0.08) 0%, transparent 60%), radial-gradient(ellipse at 80% 100%, rgba(139,92,246,0.06) 0%, transparent 60%)"
      }} />
      ...
      <header style={{
        ...
        <div style={{ maxWidth: 1280, margin: "0 auto", display: "flex", alignItems: "center", height: 56, gap: 8 }}>
          ...
          <nav style={{ display: "flex", gap: 2, flex: 1 }}>
            {NAV_ITEMS.map((item) => {
              const active = page === item.id || (page === "detail" && item.id === "opportunities");
              return (
                <button key={item.id} onClick={() => navigate(item.id)}
                  style={{
                    background: active ? "rgba(96,165,250,0.1)" : "transparent",
                    border: "none", color: active ? "#60a5fa" : "#64748b",
